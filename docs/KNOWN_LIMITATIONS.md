...
## KL-014 — Snapshot local de redes fica desatualizado após o login

**Status:** Accepted for MVP

### Comportamento atual

O `state.networks` local (usado por publicar, baixar e presença) só é
repopulado no login, ao criar uma rede, e no início da presença. Não há refresh
ao navegar nem periódico. Consequência: após ser **aprovado** numa rede criada
depois do seu login, o usuário pode precisar sair e entrar de novo para
publicar/baixar naquela rede (o snapshot ainda não a contém).

O fluxo de **pedir acesso** já não sofre disso: os use cases de acesso
(`request-network-access`, `list-network-access-requests`, `decide-network-access`)
usam o id direto contra o hub, sem validar contra o snapshot local.

### Comportamento final esperado

O client deve atualizar o catálogo de redes ao abrir a lista/detalhe e/ou
periodicamente, para que o estado local reflita mudanças de membership sem
exigir novo login.

### Impacto

Baixo. Contornável relogando. Não afeta pedir acesso nem a listagem (que já vão
ao vivo no hub).

### Revisão prevista

MVP 9 — refresh do catálogo ao navegar / polling.
